...
def train_note_duration_conv_net(test_data_arr, test_data_label, train_data_arr, train_data_label):
    """
    This function trains the convolutional network for recognizing note durations based on resources data.
...
def values_to_numerical(data_label_values, class_names):
    ...
    for label_value in data_label_values:  # Iterate through string classifications.
        ...
        if label_value in class_names:
            ...
            print("values_to_numerical ERROR! Unclassified items! Check resources stream!", label_value)
...
def analyze_using_saved_data(img_name):
    """
    This functions uses saved weights of an already trained network to recognize the notes from the resources image.
    :param img_name: Name of the resources image.
